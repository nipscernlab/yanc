...
int main(int argc, char** argv) {
    ...
    std::map<unsigned,long long> hist;   // PC -> cycles spent there
    for (long long cyc = 0; cyc < max_cycles; ++cyc) {
        ...
        if (top->req_in) top->in = in_reg;
        top->eval();
        hist[(unsigned)top->rootp->PCSIG]++;
        if (top->out_en) {            // posedge write
            printf("CYCLES %lld  value %d\n", cyc, (int)top->out);
    ...
    FILE* pf = fopen("prof.txt", "w");
    if (pf) { for (auto& kv : hist) fprintf(pf, "%u %lld\n", kv.first, kv.second); fclose(pf); }
    delete top;
    return 0;
}
