#!/bin/bash
# Elaborates SAPHO/ula.v with ONE float operator at a time, at every #FROUND
...
for L in 0 1 2; do
    for OP in F_ADD F_SU1 F_SU2 F_MLT F_DIV I2F I2F_M F2I F2I_M F_ROT F_LES F_GRE F_NEG F_ABS F_PST F_SGN F_SCL XPO; do
        if iverilog -g2012 -s ula -Pula.FROUND=$L -Pula.$OP=1 -o /dev/null SAPHO/ula.v 2>"$TMP/e.log"; then
