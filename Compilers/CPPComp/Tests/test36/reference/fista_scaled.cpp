...
static void conv_full(const float *src, const float *kern) {
    ...
    for (int i = 0; i < LX; ++i) { float xi = src[i]; for (int j = 0; j < M; ++j) s_conv[i + j] += xi * kern[j]; }
}
...
static void correlate_h() { for (int n = 0; n < LX; ++n) { float s = 0.0f; for (int k = 0; k < M; ++k) s += s_conv[n + k] * h_hat[k]; s_grad_x[n] = s; } }
static void correlate_x() { for (int n = 0; n < M; ++n) { float s = 0.0f; for (int k = 0; k < LX; ++k) s += s_conv[n + k] * x_hat[k]; s_grad_h[n] = s; } }
...
static void fista() {
    ...
    for (int it = 0; it < XIT; ++it) {
        ...
        for (int i = 0; i < LX; ++i) { float v = s_z[i] - step * s_grad_x[i] - step_lam; if (v < 0.0f) v = 0.0f; x_hat[i] = v; }
        float tn = 0.5f * (1.0f + std::sqrt(1.0f + 4.0f * t * t));
        float mom = (t - 1.0f) / tn;
        for (int i = 0; i < LX; ++i) { float xi = x_hat[i]; s_z[i] = xi + mom * (xi - s_x_prev[i]); s_x_prev[i] = xi; }
        t = tn;
    }
}
...
int main(int argc, char** argv) {
    ...
    std::fclose(f);
    fit();
    for (int j = 0; j < M; ++j) std::printf("%d ", (int)(h_hat[j] * GAIN_OUT + 0.5f));
    std::printf("\n");
    return 0;
}
